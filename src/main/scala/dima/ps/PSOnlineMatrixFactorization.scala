...
object PSOnlineMatrixFactorization {
  ...
  def psOnlineMF(src: DataStream[(Rating, W)], numFactors: Int = 10, rangeMin: Double = -0.01,
                 ...
                 iterationWaitTime: Long = 10000): DataStream[Either[(W, Double), (ItemId, Vector)]] = {
    ...
    val serverLogic = new SimplePSLogic[(ItemId, Int), Array[Double]](x => factorInitDesc.open().nextFactor(x),
